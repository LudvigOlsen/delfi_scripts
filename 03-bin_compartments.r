...
if (opt$assembly == "hg19"){
  ...
  load(file.path(opt$filters_dir, "filters.hg19.rda")); filters <- filters.hg19; remove(filters.hg19)
  load(file.path(opt$filters_dir, "gaps.hg19.rda")); gaps <- gaps.hg19; remove(gaps.hg19)
  ...
  load(file.path(opt$filters_dir, "filters.hg38.rda")); filters <- filters.hg38; remove(filters.hg38)
  load(file.path(opt$filters_dir, "gaps.hg38.rda")); gaps <- gaps.hg38; remove(gaps.hg38)
  AB <- readRDS(file = opt$in_bin_coordinates_file)
}
